    ...
    <div class="mb-6 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><p class="text-xs font-extrabold uppercase tracking-[.16em] text-blue-600">Dashboard Operator</p><h1 class="mt-2 text-2xl font-extrabold text-slate-800">Informasi Sekolah</h1><p class="mt-1 text-sm text-slate-500">Pantau pembaruan pengumpulan data dan pengumuman terbaru dari admin.</p></div><a href="{{ route('operator.pengajuan.index') }}" class="w-fit rounded-lg bg-blue-600 px-4 py-2.5 text-sm font-bold text-white hover:bg-blue-700">Menu</a></div>
        ...
        <div class="space-y-6">
            ...
            <div class="grid gap-6 lg:grid-cols-2">
                <section class="overflow-hidden rounded-2xl border border-amber-100 bg-white shadow-sm"><div class="bg-amber-50 px-5 py-4"><h2 class="font-extrabold text-amber-900">Pengumuman</h2><p class="mt-0.5 text-xs text-amber-700">Informasi terbaru dari Cabang Dinas</p></div><div class="divide-y divide-slate-100">@forelse ($pengumumans as $pengumuman)<article class="p-5"><h3 class="text-sm font-extrabold text-slate-800">{{ $pengumuman->judul }}</h3>@if($pengumuman->gambar)<img class="mt-3 max-h-64 w-full rounded-xl object-cover" src="{{ asset('storage/'.$pengumuman->gambar) }}" alt="{{ $pengumuman->judul }}">@endif<div class="mt-3 text-sm leading-6 text-slate-600 [&_a]:font-bold [&_a]:text-blue-600 [&_ol]:ml-5 [&_ol]:list-decimal [&_p]:mb-2 [&_ul]:ml-5 [&_ul]:list-disc">{!! $pengumuman->isi !!}</div>@if($pengumuman->lampiran)<a target="_blank" href="{{ asset('storage/'.$pengumuman->lampiran) }}" class="mt-3 inline-flex rounded-lg bg-amber-50 px-3 py-2 text-xs font-bold text-amber-700 hover:bg-amber-100">Buka lampiran</a>@endif<p class="mt-3 text-[11px] font-semibold text-slate-400">{{ $pengumuman->published_at?->translatedFormat('d F Y, H:i') ?? 'Baru diterbitkan' }}</p></article>@empty<div class="p-10 text-center text-sm text-slate-400">Belum ada pengumuman saat ini.</div>@endforelse</div></section>
            ...
            <section class="overflow-hidden rounded-2xl border border-violet-100 bg-white shadow-sm">
                ...
                <div class="grid gap-4 p-5 md:grid-cols-2 xl:grid-cols-3">@forelse($panduans as $panduan)<article class="rounded-xl border border-slate-200 bg-white p-4"><div class="flex items-center gap-2"><span class="rounded-full px-2.5 py-1 text-[10px] font-extrabold uppercase tracking-wider {{ $panduan->tipe === 'video' ? 'bg-rose-100 text-rose-700' : 'bg-violet-100 text-violet-700' }}">{{ $panduan->tipe }}</span></div><h3 class="mt-3 font-extrabold text-slate-800">{{ $panduan->judul }}</h3>@if($panduan->gambar)<img class="mt-3 h-36 w-full rounded-lg object-cover" src="{{ asset('storage/'.$panduan->gambar) }}" alt="{{ $panduan->judul }}">@endif@if($panduan->tipe === 'video')<a href="{{ $panduan->video_url }}" target="_blank" rel="noopener noreferrer" class="mt-4 inline-flex rounded-lg bg-rose-50 px-3 py-2 text-xs font-bold text-rose-700 hover:bg-rose-100">Tonton video →</a>@else<details class="group mt-3"><summary class="cursor-pointer text-xs font-bold text-violet-700">Baca artikel</summary><div class="mt-3 text-sm leading-6 text-slate-600 [&_a]:font-bold [&_a]:text-blue-600 [&_ol]:ml-5 [&_ol]:list-decimal [&_p]:mb-2 [&_ul]:ml-5 [&_ul]:list-disc">{!! $panduan->konten !!}</div></details>@endif@if($panduan->lampiran)<a target="_blank" href="{{ asset('storage/'.$panduan->lampiran) }}" class="mt-3 inline-flex rounded-lg bg-violet-50 px-3 py-2 text-xs font-bold text-violet-700 hover:bg-violet-100">Buka lampiran</a>@endif</article>@empty<div class="py-8 text-center text-sm text-slate-400 md:col-span-2 xl:col-span-3">Belum ada panduan yang tersedia.</div>@endforelse</div>
            </section>
        </div>
    </div>
@endsection
