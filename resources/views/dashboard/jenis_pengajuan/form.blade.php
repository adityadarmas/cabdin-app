    ...
    <div class="max-w-3xl mx-auto">
        ...
        <form id="jenis-form" class="mt-6 space-y-5" method="POST" action="{{ $jenisPengajuan->exists ? route('jenis-pengajuan.update', $jenisPengajuan) : route('jenis-pengajuan.store') }}">
            ...
            <section class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                ...
                <div class="mt-5 rounded-xl border border-blue-100 bg-blue-50 p-4"><h3 class="text-sm font-extrabold text-blue-800">Sasaran Sekolah</h3><p class="mt-1 text-xs leading-5 text-blue-700">Batasi jenis pengumpulan data ini agar hanya tampil pada operator sekolah yang sesuai.</p><div class="mt-4 grid gap-4 sm:grid-cols-2"><label class="text-sm font-bold text-slate-700">Bentuk Pendidikan<select class="mt-2 w-full rounded-lg border border-blue-200 bg-white px-3 py-2.5 font-normal" name="target_bentuk_pendidikan"><option value="semua" @selected(old('target_bentuk_pendidikan', $jenisPengajuan->target_bentuk_pendidikan ?? 'semua') === 'semua')>Semua (SMA dan SMK)</option><option value="sma" @selected(old('target_bentuk_pendidikan', $jenisPengajuan->target_bentuk_pendidikan) === 'sma')>SMA saja</option><option value="smk" @selected(old('target_bentuk_pendidikan', $jenisPengajuan->target_bentuk_pendidikan) === 'smk')>SMK saja</option></select></label><label class="text-sm font-bold text-slate-700">Status Sekolah<select class="mt-2 w-full rounded-lg border border-blue-200 bg-white px-3 py-2.5 font-normal" name="target_status_sekolah"><option value="semua" @selected(old('target_status_sekolah', $jenisPengajuan->target_status_sekolah ?? 'semua') === 'semua')>Semua (Negeri dan Swasta)</option><option value="negeri" @selected(old('target_status_sekolah', $jenisPengajuan->target_status_sekolah) === 'negeri')>Negeri saja</option><option value="swasta" @selected(old('target_status_sekolah', $jenisPengajuan->target_status_sekolah) === 'swasta')>Swasta saja</option></select></label></div></div>
